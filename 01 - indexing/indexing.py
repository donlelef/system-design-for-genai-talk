...
def main():
    ...
    embedding_response = openai_client.embeddings.create(
        input=movies_df["overview"].tolist(),
...
def transform_movie_dataframe_to_records(movies_df: pd.DataFrame) -> list[models.Record]:
    return [
        models.Record(
            id=idx,
...
def delete_and_create_collection(qdrant: QdrantClient, collection_name: str, embedding_dimensions: int = 1536):
...
def fetch_movies() -> pd.DataFrame:
    movies_df = pd.read_parquet("https://github.com/xtreamsrl/genai-for-engineers-class/raw/main/data/movies.parquet")
